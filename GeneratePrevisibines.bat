...
::      (or xPrevisPatch.esp is used to create that plugin if it doesn't exist)
:: Can be called with parameters [-clean/-filtered/-xbox] [-bsarch] [modname.esp]
...
::
:: Author: PJM V2.9 June 2025 (Add -bsarch option, support CKPE .toml config file, block unuseable modnames, Fix BSarch path)

echo =================================================================
echo Automatic Previsbine Builder (V2.9 PJM Jun 2025)
...
:: If the above fails then remove the REM in front of the following and set it to your xEdit/FO4edit path (or run this script from that folder)
REM SET FO4Edit_=C:\Program Files (x86)\FO4Edit\FO4Edit.exe
...
:: If the above fails then remove the REM in front of the following and set it to your FO4 installation (don't forget the trailing \ )
REM SET locCreationKit_=C:\Program Files (x86)\Steam\steamapps\common\Fallout 4\
...
If Not Exist "%locCreationKit_%Fallout4.exe" (echo ERROR - Fallout4 cannot be found. Run Fallout4Launcher.exe once to fix & goto PauseAndExit)
...
If "%~1" EQU "" goto ShowVers
If /I "%~1" EQU "filtered" goto SetBuildMode1
If /I "%~1" EQU "-filtered" goto SetBuildMode1
If /I "%~1" EQU "xbox" goto SetBuildMode1
If /I "%~1" EQU "-xbox" goto SetBuildMode1
If /I "%~1" EQU "clean" goto SetBuildMode1
If /I "%~1" EQU "-clean" goto SetBuildMode1
If /I "%~1" NEQ "-bsarch" goto SetPlugin1
Set ArchProg_=BSArch
If "%~2" EQU "" goto ShowVers
If /I "%~2" EQU "filtered" goto SetBuildMode2
If /I "%~2" EQU "-filtered" goto SetBuildMode2
If /I "%~2" EQU "xbox" goto SetBuildMode2
If /I "%~2" EQU "-xbox" goto SetBuildMode2
If /I "%~2" EQU "clean" goto SetBuildMode2
If /I "%~2" EQU "-clean" goto SetBuildMode2
...
SET BuildMode_=%BuildMode_:-=%
If "%~2" EQU "" goto ShowVers
If /I "%~2" NEQ "-bsarch" goto SetPlugin2
Set ArchProg_=BSArch

:SetPlugin3
If "%~3" EQU "" goto ShowVers
SET PluginName_=%~3
SET NoPrompt_=true
...
If Not Exist "%locCreationKit_%winhttp.dll" (echo ERROR - CKPE not installed. You may not get a successful Patch without it & goto PauseAndExit)
...
set CreationKitlog_=%locCreationKit_%%%~a
set CreationKitlog_=!CreationKitlog_:'=!
)
IF "%CreationKitlog_%" NEQ "" goto CheckCKPEConfig
...
echo Increased Reference Limit not enabled, Precombine Phase may fail.
...
echo Specify the name for the Patch (If no extension then assumes .esp)
...
echo [2] Merge PrecombineObjects.esp Via FO4Edit
...
echo [7] Merge Previs.esp Via FO4Edit
echo [8] Add vis files to BA2 Archive
...
:BldCDX
If /I "%BuildMode_%" EQU "clean" (Call :RunCK BuildCDX "%PluginName_%.cdx" "")
...
ECHO Creating %Arch2Quals_% Archive %PluginArchive_% of %~1
...
:Extract
ECHO Extracting Archive %PluginArchive_% 
...
ECHO Creating %Arch2Quals_% Archive %PluginArchive_% of meshes\precombined,%~1
