...
class PluginInputWidget(QWidget):
    ...
    def _init_ui(self) -> None:
        ...
        self.validation_message.setStyleSheet("QLabel { padding: 2px 5px; }")
    ...
    def _on_text_changed(self, _text: str) -> None:
        ...
        Args:
            _text: Current text in the line edit (unused, gets text from widget directly)
        """
            ...
            if plugin_path.exists():
                ...
                if archive_path.exists():
                    self._set_validation_state(
                        False, f"Archive '{archive_name}' already exists. Choose a different plugin name.", exists=False
                    )
                else:
                    self._set_validation_state(True, f"Plugin does not exist and will be created: {plugin_name}", exists=False)
        else:
            self._set_validation_state(is_valid, error_message or "Plugin name is valid")

    def _set_validation_state(self, is_valid: bool, message: str, exists: bool | None = None) -> None:
        ...
        if is_valid:
            if exists is True:
                ...
                self.validation_icon.setText("ℹ")  # noqa: RUF001
